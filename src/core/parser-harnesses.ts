...
import { findHermesDirs, parseHermesSessions } from './parser-hermes';
import { findGsdDirs, parseGsdSessions } from './parser-gsd';
...
const EXTERNAL_HARNESSES: ExternalHarnessCollector[] = [
  ...
  },
  {
    name: 'Hermes',
    collectSync(ctx) {
      for (const hermesDir of findHermesDirs()) {
        for (const session of parseHermesSessions(hermesDir)) addSession(ctx.workspaces, ctx.sessions, session, hermesDir);
      }
    },
  },
  {
    name: 'GSD-Pi',
    collectSync(ctx) {
      for (const gsdDir of findGsdDirs()) {
        for (const session of parseGsdSessions(gsdDir)) addSession(ctx.workspaces, ctx.sessions, session, gsdDir);
      }
    },
  },
];
